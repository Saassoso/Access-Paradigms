...
Service Provider (SP) côté Windows. Reçoit les assertions SAML depuis [[Authentik]] (IdP).  
...
Cela lie le device au tenant Entra. Le login Windows utilise ensuite les credentials fédérés depuis Authentik via SAML.

## Enterprise Application (fédération SAML)

Pour que Entra accepte les assertions SAML d'Authentik :

1. Entra Admin → Enterprise Applications → New Application → Create your own
2. Configure Single Sign-On → SAML
3. Renseigner :
   - **Entity ID** : correspond à l'issuer configuré dans Authentik
   - **Reply URL (ACS)** : URL vers laquelle Authentik envoie l'assertion
   - **Sign-on URL** : URL de login Entra
4. Télécharger le Certificate (Base64) → importer dans Authentik
5. Copier les metadata → importer dans Authentik Provider SAML

## App Registration (fédération OIDC)

Pour que Authentik puisse fédérer les identités Microsoft :

1. Entra Admin → App Registrations → New Registration
2. Nom : `authentik-federation`
...
5. Configurer dans Authentik : Sources → OAuth/OIDC → Microsoft/Entra

## Compte break-glass

Un compte natif Entra (pas fédéré) à utiliser si Authentik est down.  
