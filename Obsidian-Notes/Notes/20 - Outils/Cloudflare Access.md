...
| MFA | TOTP (via son propre stack) | Via keycloak OIDC (déjà en place)                     |
...
```
Name : keycloak
Client ID : <copier depuis keycloak Provider OIDC>
Client Secret : <copier depuis keycloak Provider OIDC>
...
Include : Emails ending in @charif-labs.tech
         OU groups = admins (claim keycloak)
```
### 4. Provider OIDC dans keycloak (côté IdP)
keycloak → Applications → Providers → Create → OAuth2/OIDC Provider
```
...
```
- Créer une Application keycloak liée à ce Provider.
## Vérification
```bash
...
Si tu veux protéger l'accès interne aussi → utiliser **keycloak Outpost** en forward-auth sur le réseau Docker.
