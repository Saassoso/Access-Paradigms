...
- [x] Tester manuellement la création d'un utilisateur Entra via Postman/Graph Explorer
...
- [x] Créer Service Account GCP avec Domain-Wide Delegation
  - Scope : `admin.directory.user` (restreint à OU Bureau-1)
- [x] Télécharger GCPW MSI (domaine = `google.charif-labs.tech`)
- [x] Script `bootstrap_gcpw.ps1` : installation GCPW silencieuse + Action1 agent
- [x] **Test login Windows** avec compte Google Workspace

### 2.C — Action1 (RMM commun aux deux profils)
- [x] Créer compte Action1
- [x] Créer groupe `Onboarding-Entra` (Bureau-2) + `Onboarding-GCPW` (Bureau-1)
- [x] Configurer scripts d'onboarding automatiques dans chaque groupe
  - `Apply-RegistryPolicies.ps1`
  - `Set-SecurityPolicies.ps1`
  - `Init-LAPS.ps1`
- [x] Ajouter attribut `action1_agent_id` dans Keycloak/Keycloak pour chaque utilisateur *(requis pour offboarding)*
...
### 2.D — Ansible (Hardening Docker Server)
- [ ] `ansible/inventory.ini` 
- [ ] Playbook Linux Hardening

**Gatekeeper :** 
- Un utilisateur Bureau-2 peut se connecter à Windows avec son compte Keycloak/Entra. 
- Un utilisateur Bureau-1 peut se connecter avec Keycloak/GCPW.
...
- [x] Déployer Wazuh Manager sur Docker Host
  - **OpenSearch RAM : `-Xms2g -Xmx2g` — OBLIGATOIRE**
- [x] Configurer Sysmon sur VMs Windows avec config **sysmon-modular** (pas de config vierge) avec Action 1.
- [x] Agent Wazuh déployé via Action1 (`Deploy-WazuhAgent.ps1`)
  - **Point de synchronisation responsable** : notifier avant lancement
- [x] Events Sysmon 1, 3, 10 visibles dans le dashboard Wazuh
- [x] Score SCA CIS ≥ 85%
- [x] FIM activé sur `C:\Windows\System32`
- [x] Configurer le webhook Wazuh → n8n (niveau ≥ 12 uniquement, immédiat)
- [x] Configurer le webhook Wazuh → n8n (niveau 7-11, batch 5 min pour IA)
...
- [x] Déployer n8n sur Docker Host
- [x] Configurer Cloudflare Tunnel → `n8n.charif-labs.tech`
- [x] Cloudflare Access policy → groupe `it-admin` uniquement
- [x] Configurer tous les credentials n8n (Graph API, Google SDK, Action1, LLM)
- [x] Activer validation Header Authentication sur les webhooks

### 4.2 — Workflow Provisioning 
- [x] Activer Event Listeners Keycloak/Keycloak → webhook n8n
- [x] Implémenter Switch Node (bureau-1 → Google / bureau-2 → Entra)
- [x] Branche Microsoft : Graph API — créer/update utilisateur + sync password
- [x] Branche Google : Admin SDK — créer/update utilisateur + sync password
- [x] Gestion d'erreurs : retry x3 + alerte Slack si échec persistant
- [x] Test : créer utilisateur bureau-1 dans Keycloak → compte Google créé automatiquement
- [x] Test : créer utilisateur bureau-2 dans Keycloak → compte Entra créé automatiquement

### 4.3 — Workflow Offboarding 
...
## Phase 5 — Haute Disponibilité & Extension Cloud DigitalOcean
